...
![VMs Status](screenshots/02-vms-running.png)

**Все 5 ВМ:** статус Running ✅  
**Зоны:** ru-central1-a (4 ВМ), ru-central1-b (1 ВМ)  
**Отказоустойчивость:** веб-серверы в разных зонах доступности
...
![ALB Status](screenshots/01-alb-status.png)

**Статус:** Active ✅  
**IP-адрес:** 158.160.184.65  
**HTTP-роутер:** web-router  
**Health Check:** HTTP GET / → 200 OK


### 🔷 Тест балансировки

![Load Balancer Test](screenshots/03-load-balancer-test.png)
...
|--------|-----|-------|------|-----------|------------------|
| **Сайт** | Публичный | `158.160.184.65` | 80 (HTTP) | Интернет | `curl http://158.160.184.65` |
| **Kibana** | Публичный | `178.154.253.44` | 5601 (HTTP) | Интернет | `http://178.154.253.44:5601` |
| **Grafana** | Публичный | `178.154.252.37` | 3000 (HTTP) | Интернет | `http://178.154.252.37:3000` |
| **Elasticsearch** | Приватный | `10.0.1.15` | 9200 | Только из VPC | `curl http://10.0.1.15:9200` (через bastion) |
| **Prometheus** | Приватный | `10.0.1.38` | 9090 | Только из VPC | `curl http://10.0.1.38:9090` (через bastion) |
| **SSH ко всем ВМ** | Через Bastion | `89.169.144.182` | 22 (SSH) | Только через bastion | `ssh -J bastion web-a` |
